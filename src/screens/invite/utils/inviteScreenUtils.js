export const filterAndSortAppUsers = (
  users,
  searchQuery,
  selectedGroup,
  showFavorites,
  ...
  return users
    .filter((user) => {
      ...
      if (showLocalNode && !user.isLocalNode) {
        console.log(
          `[Filter] User ${user.name} filtered out by local filter - isLocalNode:`,
          user.isLocalNode
        );
        return false;
      }
...
export const createManualContact = (
  name,
  email,
  phone,
  message,
