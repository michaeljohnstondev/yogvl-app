...
export async function saveEventTemplate(userId, template) {
  ...
  try {
    ...
    const cleanPayload = template.payload
      ? JSON.parse(
          JSON.stringify(template.payload, (key, value) => {
    ...
    console.log(
      '[TEMPLATE SAVE] Saving to subcollection:',
      `users/${userId}/eventTemplates`
    );
    console.log('[TEMPLATE SAVE] Clean template data:', clean);

    const result = await addDoc(
      collection(db, `users/${userId}/eventTemplates`),
      clean
    );
