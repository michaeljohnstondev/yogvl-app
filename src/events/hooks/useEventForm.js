...
import { uploadEventPoster, isLocalPosterUri } from '../../services/eventPosterService';
...
export const useEventForm = () => {
  ...
  const prepareEventData = useCallback(
    (formData, dateTimeValues, currentUserId, isEditing = false, studioTimezone = null) => {
...
This was auto-generated by The Yo app`;
  ...
  const sendTextInvites = useCallback(
    async (selectedContacts, eventData, vibeAlert, currentUserId, userData) => {
      ...
      try {
        ...
        const smsUrl =
          Platform.OS === 'ios'
            ? `sms:${phoneNumbers}&body=${inviteMessage}` // No encoding on iOS - Messages app handles it natively
            : `sms:${phoneNumbers}?body=${encodeURIComponent(inviteMessage)}`; // Keep encoding on Android
  ...
  const saveEventToDatabase = useCallback(
    async (
      ...
      const userStudio = userData?.userdata?.studios?.default?.studioId || 'greenville_sc'; // Default to Greenville for now
      const studioName = userData?.userdata?.studios?.default?.studioName || 'Greenville Studio';
      const studioCity = userData?.userdata?.studios?.default?.studioCity || 'Greenville';
      const studioState = userData?.userdata?.studios?.default?.studioState || 'SC';
      ...
      // Generate a unique invite code up front so the QR tab and SMS invites
      // always have one available without an extra round-trip.
      let inviteCode = null;
      try {
        const { generateUniqueInviteCode } = await import(
          '../../services/inviteCodeService'
        );
        inviteCode = await generateUniqueInviteCode();
      } catch (error) {
        console.warn('Failed to pre-generate invite code; will be created on demand:', error);
      }
      ...
      const eventDataWithStudio = {
        ...
        ...(inviteCode ? { inviteCode, inviteCodeEnabled: true } : {}),
        attendance: [
          {
  ...
  const submitEvent = useCallback(
    async ({
      ...
      studioTimezone = null, // Studio timezone for correct UTC conversion
    }) => {
      ...
      try {
        ...
        // Poster image handling: formData.posterImage can be a local picker
        // URI (file://) or an https URL. Uploads happen here so the storage
        // path can include the eventId (and for edits, the old image can be
        // cleaned up).
        const userStudioId =
          userData?.userdata?.studios?.default?.studioId || 'greenville_sc';
        const localPosterUri = isLocalPosterUri(formData.posterImage)
          ? formData.posterImage
          : null;

        // Step 4: Save to database (create or update)
        if (isEditing && eventId) {
          // For edits, upload the new poster first so its URL is part of the
          // single updateDoc call. Pass the existing URL (read from Firestore)
          // so the old file gets cleaned up.
          if (localPosterUri) {
            try {
              const { doc, getDoc } = await import('../../lib/firebase/firestore');
              const eventRef = doc(db, 'studios', userStudioId, 'events', eventId);
              const existing = await getDoc(eventRef);
              const oldUrl = existing.exists() ? existing.data()?.posterImage : null;
              ...
              eventData.posterImage = url;
            } catch (err) {
              console.warn('[useEventForm] Poster upload failed on edit:', err);
              // Fall through — event update still proceeds without poster change
            }
          }
          ...
          // For new events, upload poster now that we have an eventId, then
          // patch the doc with the resulting URL.
          if (localPosterUri && eventRef?.id) {
            try {
              const url = await uploadEventPoster(
                userStudioId,
                eventRef.id,
              ...
              const { updateDoc } = await import('../../lib/firebase/firestore');
              await updateDoc(eventRef, { posterImage: url });
            } catch (err) {
              console.warn('[useEventForm] Poster upload failed on create:', err);
              // Non-fatal — the event still exists, just without a poster
            }
          }

          // Request notification permission when user creates their first event (contextual request)
          // This follows Facebook/Instagram pattern - ask when user takes meaningful action
          notificationPermissionService.requestPermissionIfNeeded(currentUserId, 'create_event');
        }
  ...
  const updateEventInDatabase = useCallback(
    async (
      ...
      const userStudio = userData?.userdata?.studios?.default?.studioId || 'greenville_sc';
  ...
  const handleSuccessfulSubmission = useCallback((callbacks) => {
    ...
    // Show tap-to-dismiss alert (no buttons) and navigate back
