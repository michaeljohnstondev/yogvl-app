...
export const testGooglePlacesNewAPI = async () => {
  ...
  if (!isAvailable) {
    console.log(
      '❌ API key not configured. Please check src/config/googlePlaces.js'
    );
    return false;
  }
  ...
  try {
    ...
    for (const query of testQueries) {
      ...
      if (predictions.length > 0) {
        predictions.slice(0, 3).forEach((prediction, index) => {
          console.log(
            `   ${index + 1}. ${prediction.text} (${prediction.placeId})`
          );
      ...
      await LocationAnalytics.trackGooglePlacesUsage(
        'autocomplete',
    ...
    if (testQueries.length > 0) {
      ...
      const predictions =
        await GooglePlacesService.getAutocompletePredictions(firstQuery);
      ...
      if (predictions.length > 0) {
        ...
        if (details) {
          ...
          console.log(
            `   Location: ${details.location ? `${details.location.latitude}, ${details.location.longitude}` : 'N/A'}`
          );
          console.log(
            `   Types: ${details.types?.slice(0, 3).join(', ') || 'N/A'}`
          );
        } else {
    ...
    const cachedPredictions =
      await GooglePlacesService.getAutocompletePredictions('Starbucks');
    ...
    const invalidDetails = await GooglePlacesService.getPlaceDetails(
      'invalid_place_id_12345'
    );
    ...
    console.log(
      `   Google Places quota: ${performanceMetrics.googlePlacesQuota?.dailyRequests || 0}/1000 daily requests`
    );

    if (performanceMetrics.averageResponseTimes) {
      console.log('⚡ Performance Metrics:');
      Object.entries(performanceMetrics.averageResponseTimes).forEach(
        ([api, time]) => {
