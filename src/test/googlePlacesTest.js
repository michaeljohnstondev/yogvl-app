...
const testGooglePlacesAPI = async () => {
  try {
    ...
    placePredictions.slice(0, 3).forEach((prediction, index) => {
      console.log(`   ${index + 1}. ${prediction.text} (ID: ${prediction.placeId})`);
    });
    ...
    if (placePredictions.length > 0) {
      ...
      const detailsResponse = await fetch(`${BASE_URL}/${firstPrediction.placeId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'X-Goog-Api-Key': API_KEY,
      ...
      if (detailsResponse.ok) {
        ...
        console.log('✅ Place details retrieved:', placeDetails.displayName?.text);
        console.log('📍 Address:', placeDetails.formattedAddress);
        console.log('🗺️  Location:', placeDetails.location);
      } else {
        console.error('❌ Failed to get place details:', detailsResponse.status);
      }
    }
