...
export const getEmojiForText = (text) => {
  ...
  if (
    cleanText.includes('ball') &&
    !cleanText.includes('football') &&
    !cleanText.includes('basketball') &&
    !cleanText.includes('baseball') &&
    !cleanText.includes('pickleball') &&
    !cleanText.includes('volleyball')
  )
    return '🏀';
  if (cleanText.includes('clemson') || cleanText.includes('tigers'))
    return '🐾';
  if (
    cleanText.includes('game') ||
    cleanText.includes(' vs ') ||
    cleanText.includes(' v ')
  )
    return '🏟️';
  if (cleanText.includes('hockey')) return '🏒';
  if (cleanText.includes('swim') || cleanText.includes('pool')) return '🏊';
  if (
    cleanText.includes('run') ||
    cleanText.includes('jog') ||
    cleanText.includes('marathon')
  )
    return '🏃';
  if (cleanText.includes('bike') || cleanText.includes('cycling')) return '🚴';
  if (
    cleanText.includes('gym') ||
    cleanText.includes('workout') ||
    cleanText.includes('fitness')
  )
    return '💪';
  if (cleanText.includes('yoga')) return '🧘';
  if (cleanText.includes('climb') || cleanText.includes('mountain'))
    return '🧗';
  ...
  if (
    ...
    cleanText.includes('coworker') ||
    cleanText.includes('colleague')
  )
    return '👥';
  if (cleanText.includes('client') || cleanText.includes('business'))
    return '🤝';
  if (cleanText.includes('project') || cleanText.includes('deadline'))
    return '📋';
  if (cleanText.includes('conference') || cleanText.includes('seminar'))
    return '🎤';
  if (cleanText.includes('startup') || cleanText.includes('entrepreneur'))
    return '🚀';

  // Social & Friends
  if (
    cleanText.includes('friend') ||
    cleanText.includes('buddy') ||
    cleanText.includes('pal')
  )
    return '👯';
  if (cleanText.includes('family') || cleanText.includes('relative'))
    return '👨‍👩‍👧‍👦';
  if (cleanText.includes('party') || cleanText.includes('celebration'))
    return '🎉';
  ...
  if (
    ...
    cleanText.includes('film') ||
    cleanText.includes('cinema')
  )
    return '🎬';
  if (
    cleanText.includes('music') ||
    cleanText.includes('concert') ||
    cleanText.includes('band')
  )
    return '🎵';
  if (cleanText.includes('jazz')) return '🎷';
  if (cleanText.includes('rock') || cleanText.includes('classic rock'))
    return '🎸';
  if (cleanText.includes('edm') || cleanText.includes('electronic'))
    return '🎧';
  ...
  if (
    cleanText.includes('art') ||
    cleanText.includes('paint') ||
  ...
  if (cleanText.includes('board game') || cleanText.includes('board'))
    return '🎲';
  if (cleanText.includes('card game') || cleanText.includes('cards'))
    return '🃏';
  if (cleanText.includes('poker')) return '♠️';
  if (
    cleanText.includes('garden') ||
    cleanText.includes('plant') ||
    cleanText.includes('green thumb')
  )
    return '🌱';
  if (
    cleanText.includes('cook') ||
    cleanText.includes('chef') ||
    cleanText.includes('recipe')
  )
    return '👨‍🍳';
  if (cleanText.includes('dancing') || cleanText.includes('dance')) return '💃';
  if (
    cleanText.includes('clubbing') ||
    cleanText.includes('club') ||
    cleanText.includes('nightclub')
  )
    return '🕺';
  if (cleanText.includes('book club')) return '📚';
  if (cleanText.includes('investment') || cleanText.includes('investing'))
    return '📈';
  if (cleanText.includes('entrepreneur') || cleanText.includes('startup'))
    return '🚀';
  ...
  if (cleanText.includes('breakfast') || cleanText.includes('brunch'))
    return '🥐';
  if (cleanText.includes('lunch')) return '🥙';
  if (cleanText.includes('dinner') || cleanText.includes('restaurant'))
    return '🍽️';
  if (cleanText.includes('potluck')) return '🥘';
  if (cleanText.includes('family dinner')) return '👨‍👩‍👧‍👦';
  if (cleanText.includes('out for dinner') || cleanText.includes('dining out'))
    return '🍽️';
  if (cleanText.includes('pizza')) return '🍕';
  if (cleanText.includes('burger') || cleanText.includes('bbq')) return '🍔';
  if (cleanText.includes('sushi') || cleanText.includes('japanese'))
    return '🍣';
  if (cleanText.includes('taco') || cleanText.includes('mexican')) return '🌮';
  if (
    cleanText.includes('beer') ||
    cleanText.includes('drinks') ||
    cleanText.includes('bar')
  )
    return '🍺';
  if (cleanText.includes('drinking') || cleanText.includes('cocktails'))
    return '🍻';
  if (cleanText.includes('wine') || cleanText.includes('vineyard')) return '🍷';

  // Travel & Places
  if (
    cleanText.includes('beach') ||
    cleanText.includes('ocean') ||
    cleanText.includes('surf')
  )
    return '🏖️';
  if (cleanText.includes('camping') || cleanText.includes('camp')) return '🏕️';
  if (
    cleanText.includes('hiking') ||
    cleanText.includes('hike') ||
    cleanText.includes('trail')
  )
    return '🥾';
  if (
    cleanText.includes('travel') ||
    cleanText.includes('trip') ||
    cleanText.includes('vacation')
  )
    return '✈️';
  ...
  // Events & Occasions
  if (cleanText.includes('grad') || cleanText.includes('graduation'))
    return '🎓';
  if (cleanText.includes('holiday') || cleanText.includes('christmas'))
    return '🎄';
  if (cleanText.includes('halloween')) return '🎃';
  if (cleanText.includes('new year')) return '🎊';
  if (cleanText.includes('volunteer') || cleanText.includes('charity'))
    return '🤲';
  if (
    cleanText.includes('study') ||
...
export const getContextualSuggestions = (input, context = 'general') => {
  ...
  const matches = (keyword) => {
    return (
      ...
      keyword.startsWith(cleanInput) ||
      keyword.includes(cleanInput)
    );
  };
  ...
  if (matches('fantasy football') || matches('fantasy'))
    suggestions.push('Fantasy Football');
  if (matches('football')) suggestions.push('Football');
  if (matches('basketball')) suggestions.push('Basketball');
  if (
    matches('ball') &&
    !cleanInput.includes('football') &&
    !cleanInput.includes('basketball')
  )
    suggestions.push('Basketball');
  ...
  if (matches('running') || matches('run') || matches('jog'))
    suggestions.push('Running');
  ...
  // Work suggestions
  if (cleanInput.includes('work') || cleanInput.includes('office'))
    suggestions.push('Work');
  if (cleanInput.includes('meeting') || cleanInput.includes('conference'))
    suggestions.push('Meeting');
  if (cleanInput.includes('team') || cleanInput.includes('coworker'))
    suggestions.push('Team');
  if (cleanInput.includes('project')) suggestions.push('Project');
  if (cleanInput.includes('client') || cleanInput.includes('business'))
    suggestions.push('Business');
  if (cleanInput.includes('investment') || cleanInput.includes('investing'))
    suggestions.push('Investment');
  if (cleanInput.includes('entrepreneur') || cleanInput.includes('startup'))
    suggestions.push('Entrepreneur');
  if (cleanInput.includes('real estate')) suggestions.push('Real Estate');

  // Social suggestions
  if (cleanInput.includes('friend') || cleanInput.includes('buddy'))
    suggestions.push('Friends');
  if (cleanInput.includes('family')) suggestions.push('Family');
  if (cleanInput.includes('party') || cleanInput.includes('celebration'))
    suggestions.push('Party');
  ...
  if (cleanInput.includes('breakfast') || cleanInput.includes('brunch'))
    suggestions.push('Brunch');
  ...
  if (
    cleanInput.includes('out for dinner') ||
    cleanInput.includes('dining out')
  )
    suggestions.push('Dinner Out');
  if (cleanInput.includes('pizza')) suggestions.push('Pizza');
  if (cleanInput.includes('burger') || cleanInput.includes('bbq'))
    suggestions.push('BBQ');
  if (cleanInput.includes('drinks') || cleanInput.includes('bar'))
    suggestions.push('Drinks');
  if (cleanInput.includes('drinking') || cleanInput.includes('cocktails'))
    suggestions.push('Drinking');

  // Travel suggestions
  if (cleanInput.includes('beach')) suggestions.push('Beach');
  if (cleanInput.includes('camping')) suggestions.push('Camping');
  if (cleanInput.includes('hiking') || cleanInput.includes('hike'))
    suggestions.push('Hiking');
  if (cleanInput.includes('travel') || cleanInput.includes('trip'))
    suggestions.push('Trip');
  if (cleanInput.includes('vacation')) suggestions.push('Vacation');

  // Activity suggestions
  if (cleanInput.includes('movie') || cleanInput.includes('film'))
    suggestions.push('Movie');
  if (cleanInput.includes('music') || cleanInput.includes('concert'))
  ...
  if (cleanInput.includes('edm') || cleanInput.includes('electronic'))
    suggestions.push('EDM');
  if (cleanInput.includes('house music')) suggestions.push('House Music');
  if (cleanInput.includes('rap') || cleanInput.includes('hip hop'))
    suggestions.push('Rap');
  if (cleanInput.includes('karaoke')) suggestions.push('Karaoke');
  if (cleanInput.includes('game') || cleanInput.includes('gaming'))
    suggestions.push('Gaming');
  if (cleanInput.includes('board game') || cleanInput.includes('board'))
    suggestions.push('Board Games');
  if (cleanInput.includes('card game') || cleanInput.includes('cards'))
    suggestions.push('Card Games');
  if (cleanInput.includes('poker')) suggestions.push('Poker');
  if (cleanInput.includes('book') || cleanInput.includes('read'))
    suggestions.push('Book Club');
  if (cleanInput.includes('art') || cleanInput.includes('paint'))
    suggestions.push('Art');
  if (cleanInput.includes('painting')) suggestions.push('Painting');
  if (cleanInput.includes('dancing') || cleanInput.includes('dance'))
    suggestions.push('Dancing');
  if (cleanInput.includes('clubbing') || cleanInput.includes('club'))
    suggestions.push('Clubbing');
  if (cleanInput.includes('green thumb') || cleanInput.includes('garden'))
    suggestions.push('Gardening');
  ...
  if (context === 'event') {
    ...
    if (cleanInput.includes('expo') || cleanInput.includes('exhibition'))
  ...
  return [...new Set(suggestions)].slice(0, 5).map((suggestion) => ({
    text: suggestion,
...
export const getGroupSuggestions = (input) =>
  getContextualSuggestions(input, 'group').map((s) => s.text);
