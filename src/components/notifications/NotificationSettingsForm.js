// Shared notification settings form component used by both CreateEventScreen and NotificationSettingsScreen
...
export default function NotificationSettingsForm({
  ...
  const handleShowAddForm = () => {
    ...
    // Scroll down just enough to show the form
    setTimeout(() => {
      if (scrollViewRef?.current) {
        scrollViewRef.current.scrollTo({
          y: 450, // Scroll down 450px to show the form better
          animated: true,
        });
      }
    }, 150);
  ...
  const addCustomReminder = () => {
    ...
    const unitText = amount === 1 ? customUnit.slice(0, -1) : customUnit;
    const newReminder = {
      id: `custom_${Date.now()}`,
      amount,
      unit: customUnit,
  ...
  return (
    <>
      ...
      {(!showMasterToggle || settings?.enabled) && (
        <>
          ...
          {showSaveAsDefaults && onSaveAsDefaults && (
            <View style={[styles.section, sectionStyle]}>
              <TouchableOpacity
