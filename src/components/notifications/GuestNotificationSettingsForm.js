...
export default function GuestNotificationSettingsForm({
  ...
  const getCurrentTemplates = () => {
    ...
    console.log('[GuestNotificationSettings] getCurrentTemplates - templates from settings:', templates.length, templates);
    console.log('[GuestNotificationSettings] isLoadingTemplates:', isLoadingTemplates);
    
    // If no templates exist, return defaults (for new users)
    if (templates.length === 0) {
      console.log('[GuestNotificationSettings] No templates, returning defaults');
  ...
  const deleteCustomReminder = async (template) => {
    ...
    if (currentUserId && template.id.startsWith('custom_')) {
      try {
        console.log('[GuestNotificationSettings] Auto-removing custom reminder from global store:', template.label);
        await CustomTemplateService.removeCustomTemplate(currentUserId, template.id);
      } catch (error) {
        console.warn('[GuestNotificationSettings] Failed to auto-remove custom reminder from global store:', error);
  ...
  const addCustomReminder = async () => {
    if (isAddingReminder) {
      console.log('[GuestNotificationSettings] Already adding reminder, ignoring click');
    ...
    try {
      ...
      if (currentUserId) {
        try {
          console.log('[GuestNotificationSettings] Auto-saving custom reminder to global store:', newReminder.label);
          await CustomTemplateService.saveCustomTemplates(currentUserId, [newReminder]);
        } catch (error) {
          console.warn('[GuestNotificationSettings] Failed to auto-save custom reminder to global store:', error);
          // Don't show error to user - local functionality still works
        }
      }
