 ...
 */
export const reportUser = async (reporterId, targetUserId, reason = 'Inappropriate content') => {
  try {
    ...
    console.log(`[reportingService] Reported user studio: ${reportedUserStudio}`);
    console.log(`[reportingService] Full user data structure:`, userData?.userdata?.studios);
    
    if (!reportedUserStudio) {
      console.error('[reportingService] Reported user has no studio association:', userData);
    ...
    const reportData = {
      ...
      reporterInfo: {
        ...
        email: reporterData?.userdata?.contactInfo?.email || reporterData?.email || 'Unknown'
      },
      ...
      reportedUser: {
        ...
        userData: userData,
        reportedInfo: {
          name: `${userData?.userdata?.contactInfo?.firstName || ''} ${userData?.userdata?.contactInfo?.lastName || ''}`.trim() || 'Unknown User',
          email: userData?.userdata?.contactInfo?.email || userData?.email || 'Unknown'
        }
      }
    };

    // Add to reported user's studio admin collection
    const adminCollectionRef = collection(db, 'studios', reportedUserStudio, 'admin');
    const reportDoc = await addDoc(adminCollectionRef, reportData);

    console.log(`[reportingService] User report created: ${reportDoc.id} in studio: ${reportedUserStudio}`);
 ...
 */
export const reportEvent = async (reporterId, eventId, eventStudioId, reason = 'Inappropriate content') => {
  try {
    ...
    const reportData = {
      ...
      reportedEvent: {
        ...
        creatorInfo: {
          ...
          name: `${creatorData?.userdata?.contactInfo?.firstName || ''} ${creatorData?.userdata?.contactInfo?.lastName || ''}`.trim() || 'Unknown User',
          email: creatorData?.userdata?.contactInfo?.email || creatorData?.email || 'Unknown'
        }
      }
    };

    // Add to event's studio admin collection
    const adminCollectionRef = collection(db, 'studios', eventStudioId, 'admin');
    const reportDoc = await addDoc(adminCollectionRef, reportData);

    console.log(`[reportingService] Event report created: ${reportDoc.id} in studio: ${eventStudioId}`);
 ...
 */
export const updateReportStatus = async (studioId, reportId, status, adminNotes = '') => {
  try {
    ...
    console.log(`[reportingService] Report ${reportId} status updated to: ${status}`);
...
export const getStudioReports = async (studioId, limit = 50) => {
  try {
    ...
    console.log(`[getStudioReports] Found ${snapshot.docs.length} documents in /studios/${studioId}/admin`);
