  ...
  try {
    ...
    try {
      ...
      for (const user of users) {
        const isUserFollowingMe = await checkIfFollowing(user.id, currentUserId);
    ...
    console.log(`[userService] Found ${users.length} studio users with follow status`);
    return users;
 ...
 */
export const searchUsers = async (searchQuery, currentUserId, userStudio, limit = 20) => {
  try {
...
export const registerUserWithStudio = async (userId, studioId) => {
  try {
    if (!userId || !studioId) {
      console.warn('[userService] Missing userId or studioId for studio registration');
    ...
    try {
      ...
      if (updateError.code === 'not-found') {
        console.log(`[userService] Studio ${studioId} doesn't exist, creating it...`);
        await setDoc(studioRef, {
          id: studioId,
          name: studioId.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()), // Format ID as name
          users: [userId],
          createdAt: new Date(),
          lastUpdated: new Date(),
        });
    ...
    console.error(`[userService] Error registering user ${userId} with studio ${studioId}:`, error);
...
export const removeUserFromStudio = async (userId, studioId) => {
  try {
    if (!userId || !studioId) {
      console.warn('[userService] Missing userId or studioId for studio removal');
      return { success: false, error: 'Missing parameters' };
    }
    ...
    console.error(`[userService] Error removing user ${userId} from studio ${studioId}:`, error);
    return { success: false, error: error.message };
  }
};
...
export const switchUserStudio = async (userId, oldStudioId, newStudioId) => {
  try {
    if (!userId || !newStudioId) {
      console.warn('[userService] Missing userId or newStudioId for studio switch');
      return { success: false, error: 'Missing parameters' };
    }

    console.log(`[userService] Switching user ${userId} from studio ${oldStudioId} to ${newStudioId}`);
    ...
    if (oldStudioId && oldStudioId !== newStudioId) {
      ...
      if (!removeResult.success) {
        console.warn(`[userService] Failed to remove user from old studio ${oldStudioId}:`, removeResult.error);
    ...
    if (!addResult.success) {
      return { success: false, error: `Failed to register with new studio: ${addResult.error}` };
    }

    console.log(`[userService] Successfully switched user ${userId} to studio ${newStudioId}`);
    return { success: true };
    
  } catch (error) {
    console.error(`[userService] Error switching user ${userId} studios:`, error);
    return { success: false, error: error.message };
  }
};