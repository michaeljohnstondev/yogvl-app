...
export class EventEndNotificationService {
      ...
      for (const eventDoc of querySnapshot.docs) {
        ...
        if (now >= eventEndWithBuffer) {
          try {
            ...
            const result = await this.sendAttendanceNotificationToHost(eventId, eventData);
  ...
  static async sendAttendanceNotificationToHost(eventId, eventData) {
    try {
      ...
      const attendeeCount = eventData.subscribers ? eventData.subscribers.length : 0;
  ...
  static getEstimatedEventDuration(eventData) {
    ...
    if (durationMatch) {
      ...
      if (hours > 0 && hours <= 12) { // Reasonable range
