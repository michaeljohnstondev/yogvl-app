...
export class GooglePlacesService {
  ...
  static async getAutocompletePredictions(input, options = {}) {
    if (!this.isAvailable()) {
      console.log('[GooglePlacesService] API not available, skipping autocomplete');
      return [];
    }
    ...
    try {
      ...
      console.log('[GooglePlacesService] Fetching autocomplete predictions for:', input);
      ...
      console.log(`[GooglePlacesService] Found ${formattedPredictions.length} predictions`);
      ...
      LocationErrorHandler.logError('google_places_autocomplete', errorInfo, { input });
      
      console.error('[GooglePlacesService] Autocomplete error:', LocationErrorHandler.createUserMessage(errorInfo, true));
      return [];
    }
  }
  ...
  static async getPlaceDetails(placeId) {
    if (!this.isAvailable()) {
      console.log('[GooglePlacesService] API not available, skipping place details');
      return null;
    }
    ...
    try {
      ...
      console.log('[GooglePlacesService] Got place details:', placeDetails.name);
      this.setCachedResult(cacheKey, placeDetails);
      
      return placeDetails;
    } catch (error) {
      const errorInfo = LocationErrorHandler.handleGooglePlacesError(error);
      LocationErrorHandler.logError('google_places_details', errorInfo, { placeId });
      
      console.error('[GooglePlacesService] Place details error:', LocationErrorHandler.createUserMessage(errorInfo, true));
      return null;
    }
  }
  ...
  static async searchNearby(location, query = '', radius = 5000) {
    if (!this.isAvailable()) {
      console.log('[GooglePlacesService] API not available, skipping nearby search');
      return [];
    }
    ...
    try {
      ...
      console.log('[GooglePlacesService] Searching nearby places:', { location, query, radius });
      ...
      const errorInfo = LocationErrorHandler.handleGooglePlacesError(error);
      LocationErrorHandler.logError('google_places_nearby', errorInfo, { location, query, radius });
      
      console.error('[GooglePlacesService] Nearby search error:', LocationErrorHandler.createUserMessage(errorInfo, true));
      return [];
    }
  }
  ...
  static async getCurrentLocation() {
    if (!this.isAvailable()) {
      console.log('[GooglePlacesService] API not available, skipping geolocation');
      return null;
    }
    ...
    try {
      ...
      console.log('[GooglePlacesService] Requesting location from Google Geolocation API...');
      const response = await fetch(`https://www.googleapis.com/geolocation/v1/geolocate?key=${config.apiKey}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.location) {
        ...
        console.log('[GooglePlacesService] Location obtained from Google:', location);
      ...
      console.error('[GooglePlacesService] Geolocation error:', LocationErrorHandler.createUserMessage(errorInfo, true));
      return null;
    }
  }
