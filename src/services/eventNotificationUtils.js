...
export const addUserToEventNotifications = async (eventData, newUserId) => {
  try {
    console.log(`[EventNotificationUtils] Adding user ${newUserId} to scheduled notifications for event ${eventData.id}`);
    ...
    if (eventTime <= new Date()) {
      console.log(`[EventNotificationUtils] Event ${eventId} is in the past, not scheduling notifications`);
    ...
    for (const interval of intervals) {
      ...
      if (reminderTime <= now) {
        console.log(`Skipping ${interval.minutes}min reminder - time has passed`);
        continue;
      }

      const scheduleId = await ScheduledNotificationService.scheduleNotification({
        userId: newUserId,
        eventId,
    ...
    console.log(`[EventNotificationUtils] ✅ Scheduled ${notifications.length} reminder notifications for new user`);
    ...
    console.error('[EventNotificationUtils] Error adding user to event notifications:', error);
...
export const removeUserFromEventNotifications = async (eventId, userId) => {
  try {
    console.log(`[EventNotificationUtils] Removing user ${userId} from scheduled notifications for event ${eventId}`);
    ...
    if (snapshot.empty) {
      console.log(`[EventNotificationUtils] No pending notifications found for user ${userId} and event ${eventId}`);
    ...
    console.log(`[EventNotificationUtils] ✅ Cancelled ${snapshot.size} scheduled notifications for user who left event`);
    ...
    console.error('[EventNotificationUtils] Error removing user from event notifications:', error);
    return { success: false, error: error.message };
  }
};
 ...
 */
export const updateEventNotifications = async (eventId, updatedEventData, changes = []) => {
  try {
    console.log(`[EventNotificationUtils] Updating scheduled notifications for event ${eventId}`);
    ...
    if (changes.includes('datetime') || changes.includes('eventTimestamp')) {
      console.log(`[EventNotificationUtils] Event time changed, rescheduling all notifications`);
      
      // Cancel existing notifications
      await ScheduledNotificationService.cancelEventNotifications(eventId, 'Event time changed');
      
      // Reschedule with new time
      const result = await ScheduledNotificationService.scheduleEventReminders(updatedEventData);
    ...
    if (changes.includes('title')) {
      console.log(`[EventNotificationUtils] Event title changed, updating notification messages`);
      ...
      if (!snapshot.empty) {
        ...
        snapshot.docs.forEach(doc => {
          const data = doc.data();
          const updatedMessage = data.message.replace(/[""].*[""]/, `"${updatedEventData.title}"`);
    ...
    console.error('[EventNotificationUtils] Error updating event notifications:', error);
    return { success: false, error: error.message };
  }
};
